...
// Not Found Videos Modal - Selection list with glass + mobile bottom sheet
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        <div className="p-5 border-b border-white/10">
          <div className="flex items-center justify-between">
            <h3 className="text-base font-medium text-text-primary">Unavailable Videos</h3>
        ...
        <div className="flex-1 overflow-y-auto p-5">
          {!hasVideos ? (
            <p className="text-sm text-text-muted text-center py-8">✓ No videos to remove</p>
          ) : (
            <>
              <label className="flex items-center gap-2 text-sm text-text-muted mb-4 cursor-pointer">
                <input
                  ...
                  onChange={(e) => setSelectedVideos(e.target.checked ? videos.map(v => v.id) : [])}
                  className="rounded"
                />
              ...
              <div className="space-y-2">
                {videos.map((video) => (
                  <label key={video.id} className="flex items-start gap-3 p-3 bg-white/5 hover:bg-white/10 rounded-xl cursor-pointer transition-colors">
                    <input
                      type="checkbox"
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-text-muted">{video.channel_name}</div>
                    </div>
                  </label>
                ))}
              </div>
        ...
        <div className="p-5 border-t border-white/10 flex gap-2">
          ...
          {hasVideos && (
            ...
            >
              {isRemoving ? 'Removing...' : `Remove (${selectedVideos.length})`}
            </button>
          )}
        </div>
      </div>
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/10">
          <h3 className="font-semibold text-text-primary">Unavailable Videos</h3>
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {!hasVideos ? (
            <p className="text-text-muted text-center py-8">✓ No videos to remove</p>
          ) : (
            <>
              ...
              <div className="space-y-2">
                {videos.map((video) => (
                  <label key={video.id} className="flex items-start gap-3 p-4 bg-white/5 active:bg-white/10 rounded-2xl cursor-pointer">
                    <input
                      type="checkbox"
                      checked={selectedVideos.includes(video.id)}
                      onChange={(e) => {
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-text-muted mt-0.5">{video.channel_name}</div>
                    </div>
                  </label>
                ))}
              </div>
...
// Shrink Database Modal - Selection list with glass + mobile bottom sheet
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        <div className="p-5 border-b border-white/10">
          <div className="flex items-center justify-between">
            <h3 className="text-base font-medium text-text-primary">Purge Channels</h3>
            <button onClick={onClose} className="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-white/10 text-text-muted hover:text-text-primary transition-colors">
              <CloseIcon />
            </button>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-5">
          {!hasChannels ? (
            <p className="text-sm text-text-muted text-center py-8">✓ No channels to purge</p>
          ) : (
            <>
              <p className="text-sm text-text-muted mb-4">Select empty channels to permanently remove:</p>
              <div className="space-y-2">
                {channels.map((channel) => (
                  <label key={channel.id} className="flex items-start gap-3 p-3 bg-white/5 hover:bg-white/10 rounded-xl cursor-pointer transition-colors">
                    <input
                      type="checkbox"
                    ...
                    />
                    <div className="flex-1 min-w-0">
                      <div className="text-sm text-text-primary truncate">{channel.title}</div>
                      <div className="text-xs text-text-muted">{channel.video_count} video{channel.video_count !== 1 ? 's' : ''} • No library videos</div>
                    </div>
                  </label>
                ))}
              </div>
        ...
        <div className="p-5 border-t border-white/10 flex gap-2">
          ...
          {hasChannels && (
            <button
              ...
              className="flex-1 py-2.5 rounded-xl bg-red-500/90 hover:bg-red-500 text-white text-sm font-medium transition-colors disabled:opacity-50"
            >
              {isRemoving ? 'Purging...' : `Purge (${selectedChannels.length})`}
            </button>
          )}
        </div>
      </div>
      ...
      >
        <div className="w-10 h-1 bg-white/20 rounded-full mx-auto mt-3" />
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/10">
          <h3 className="font-semibold text-text-primary">Purge Channels</h3>
          <button onClick={onClose} className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center">
            <CloseIcon className="w-4 h-4 text-text-secondary" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {!hasChannels ? (
            <p className="text-text-muted text-center py-8">✓ No channels to purge</p>
          ) : (
            <>
              <p className="text-text-muted text-sm mb-4">Select empty channels to permanently remove:</p>
              <div className="space-y-2">
                {channels.map((channel) => (
                  <label key={channel.id} className="flex items-start gap-3 p-4 bg-white/5 active:bg-white/10 rounded-2xl cursor-pointer">
                    <input
                      type="checkbox"
                      checked={selectedChannels.includes(channel.id)}
                      onChange={(e) => {
                    ...
                    />
                    <div className="flex-1 min-w-0">
                      <div className="text-text-primary truncate">{channel.title}</div>
                      <div className="text-xs text-text-muted mt-0.5">{channel.video_count} video{channel.video_count !== 1 ? 's' : ''}</div>
                    </div>
                  </label>
                ))}
              </div>
