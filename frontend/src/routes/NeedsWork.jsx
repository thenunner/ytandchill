...
import { useVideos, useAddToQueue, useBulkUpdateVideos, useQueue } from '../api/queries';
    ...
    try {
      switch (action) {
        case 'queue':
          ...
          showNotification(`${selectedVideos.length} video${selectedVideos.length > 1 ? 's' : ''} added to queue`, 'success');
          break;
        case 'ignore':
          ...
          showNotification(`${selectedVideos.length} video${selectedVideos.length > 1 ? 's' : ''} ignored`, 'success');
          break;
      }
      setSelectedVideos([]);
    } catch (error) {
    ...
    <div className="space-y-6">
      <div className="sticky top-[100px] z-40 bg-dark-primary/95 backdrop-blur-lg -mx-8 px-8 py-4 mb-4 space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-3xl font-bold text-gray-100">Needs Work</h2>
            <p className="text-gray-400 mt-1">Discovered videos not yet downloaded</p>
          </div>
      ...
      {sortedVideos.length === 0 ? (
        <div className="text-center py-12 text-gray-400">
          <p className="text-lg">No videos need attention</p>
          <p className="text-sm mt-2">All discovered videos have been processed</p>
        </div>
      ) : viewMode === 'grid' ? (
